			...
			if DoesEntityExist(closestObject) then
				...
				if GetDistanceBetweenCoords(pedCoords, litCoords, true) <= 5.0 then
					if GetDistanceBetweenCoords(pedCoords, sitCoords, true) <= 2.0 and not IsEntityPlayingAnim(PlayerPedId(), 'anim@heists@box_carry@', 'idle', 3) then
						...
						if IsControlJustPressed(0, Config.Press.do_action) and not IsEntityPlayingAnim(PlayerPedId(), 'anim@heists@box_carry@', 'idle', 3) then
							WarMenu.OpenMenu('hopital')
						end
					elseif IsEntityAttachedToEntity(closestObject, GetPlayerPed(-1)) == false and not IsEntityPlayingAnim(PlayerPedId(), 'anim@heists@box_carry@', 'idle', 3) and PlayerData.job.name == "ambulance" then
						...
						if GetDistanceBetweenCoords(pedCoords, pickupCoords2, true) <= 1.5 and PlayerData.job.name == "ambulance" then
							hintToDisplay(Config.Language.take_bed)
							if IsControlJustPressed(0, Config.Press.take_bed) then
								SetVehicleExtra(closestObject, 1, 0)
								SetVehicleExtra(closestObject, 2, 1)
						...
						if not IsEntityPlayingAnim(PlayerPedId(), 'anim@heists@box_carry@', 'idle', 3) then
							...
							if IsEntityAttachedToEntity(prop, GetPlayerPed(-1)) ~= 0 or prop ~= 0 then
								...
								RegisterCommand("maca", function()
									if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), coords.x , coords.y, coords.z, true) <= 1.0 and PlayerData.job.name == "ambulance" then
...
RegisterNetEvent('Cali-Maca:removeMaca', function()
	local PlayerData = QBCore.Functions.GetPlayerData()
	...
	if PlayerData.job.name == "ambulance" then
		...
		if veh ~= 0 then
			...
			for _, v in pairs(vehicles) do
				if #(pcoords - GetEntityCoords(v)) <= 1.5 then
					if GetEntityModel(v) == -665859588 then
